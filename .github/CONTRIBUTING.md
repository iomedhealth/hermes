...
Please adhere to the DARWIN EU / OHDSI code style when adding any new code. Do not restyle any code unrelated to your pull request as this will make code review more difficult.

- **Functions & Arguments:** Use `lowerCamelCase` (e.g., `addInpatientHcru()`, `computeHospitalizationCohorts()`, `indexDate = "cohort_start_date"`).
- **Database/Cohort Columns:** Use `snake_case` (e.g., `cohort_start_date`, `inpatient_admissions`, `total_paid`).

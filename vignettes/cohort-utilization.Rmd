...
Following the **DARWIN EU / OHDSI** package ecosystem standards (`CohortConstructor`, `PatientProfiles`, `CohortCharacteristics`), **omopHeor** provides a pipe-friendly, 3-layer architecture for cohort construction, in-database column enrichment, and standardized reporting.
...
We initialize an OMOP CDM reference (such as the mock omopHeor test database):
...
`CohortUtilisation` and `CohortCosts` provide `plotUtilization()` and `plotCosts()` for rapid exploratory and publication visualization, leveraging standard `visOmopResults` ggplot themes and facets.
